...
research requires scripted, validated access to these series rather than
manual downloads from the SIDRA website.
...
structured Aggregates API v3 with JSON metadata endpoints, tidy outputs,
argument validation, in-memory caching, robust HTTP handling via `httr2`
\[@httr2\], and automatic chunking that removes the practical size
ceiling for large extractions (e.g., time series for all municipalities
in a single call).
...
automatic chunking feature, and the IBGE for maintaining the public APIs
on which this package relies.
