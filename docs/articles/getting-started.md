...
tells you everything about its structure:
...
is the main workhorse. It sends a single request and returns a tidy
tibble:
...
> 1705 has data for N1, N2, and N3 but not N6. Use
> [`ibge_metadata()`](https://monitoramento.sepe.pe.gov.br/ibger/reference/ibge_metadata.md)
> to check.
...
> Period codes encode both the date and the periodicity — `202001` could
> mean January 2020 (monthly), Q1 2020 (quarterly), or S1 2020
> (semi-annual), depending on the aggregate.

## Filtering with classifications

Many aggregates break their data further by classifications
(dimensions). For instance, aggregate 1712 (crop production) has a
classification for the type of product (226) and another for the
producer condition (218).
