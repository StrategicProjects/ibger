...
`ibge_variables()` estimates the result size and, when needed, splits the
request (by periods, then by localities) and binds the pieces back
together — see the `chunk` argument in `?ibge_variables`. The full
breakdown for one period and Brazil fits in a single request:
...
categories to what you actually need keeps the queries fast: the IBGE API
takes noticeably longer to answer large requests.
