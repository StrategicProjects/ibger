...
<!--
api-concepts.Rmd is generated from api-concepts.Rmd.orig by
...
All output below comes from running the code against the live API when the
vignette was last built (date at the end), so counts and latest periods
...
- **1712** — Produção, venda, valor da produção e área colhida da lavoura
  temporária (temporary crops, 2006 Agricultural Census)
- **7060** — IPCA — Variação mensal, acumulada no ano, acumulada em 12 meses
  e peso mensal (consumer price index)
...
sent (the API ignores what it cannot parse and returns the whole catalog):
...
condition" (218), "economic activity group", and so on. Classification 226
has categories like "pineapple" (4844), "garlic" (96608) and "potato"
(96609):
...
region". ibger passes this through directly (aggregate 512 is the harvested
area of permanent crops in the 1995 Agricultural Census):
...
the limit ibger uses. For example, a request for aggregate 2654 (deaths by
month, nature, sex and age) with:
...
You do not need to split large requests yourself. `ibge_variables()`
estimates the size of the result from the metadata (fetching the period
and locality lists when needed) and, when the estimate exceeds the limit,
splits the query — first by periods, then by localities — and binds the
pieces back together. The `chunk` argument controls this: `TRUE` (the
default) uses the built-in limit, `FALSE` forces a single request, and a
number sets a custom per-request limit. Lowering the limit is a quick way
to see the mechanism at work on a small query (27 states × 12 years = 324
values):
...
sidrar package), `parse_sidra_url()` translates it into the equivalent
`ibge_variables()` call and `fetch_sidra_url()` runs it.

## Further reading

- [IBGE API documentation](https://servicodados.ibge.gov.br/api/docs/agregados?versao=3)
- [SIDRA portal](https://sidra.ibge.gov.br/)
- [IBGE Query Builder](https://servicodados.ibge.gov.br/api/docs/agregados?versao=3#api-bq) — useful for exploring tables before writing R code
