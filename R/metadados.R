...
ibge_metadata <- function(aggregate) {
  cache_key <- paste0("meta_", aggregate)

  if (exists(cache_key, envir = .ibger_cache)) {
    result <- get(cache_key, envir = .ibger_cache)
    report_metadata_summary(aggregate, result, cached = TRUE)
    return(result)
  }
  ...
  result <- list(
    ...
    periodicity       = parse_meta_periodicity(data[["periodicidade"]]),
    territorial_level = parse_meta_territorial(data[["nivelTerritorial"]]),
    variables         = parse_meta_variables(data[["variaveis"]]),
    classifications   = parse_meta_classifications(data[["classificacoes"]])
  )
  ...
  report_metadata_summary(aggregate, result, cached = FALSE)

  result
}

#' Success message with variable/classification/category counts
#' @noRd
report_metadata_summary <- function(aggregate, result, cached) {
  n_vars <- nrow(result$variables)
  n_cls  <- nrow(result$classifications)
  n_cats <- sum(purrr::map_int(result$classifications$categories, nrow))
  suffix <- if (cached) " (cached)." else "."
  cli::cli_alert_success(
    "Aggregate {aggregate}: {n_vars} variable{?s},
     {n_cls} classification{?s}, {n_cats} categor{?y/ies}{suffix}"
  )
}

#' Parse the `variaveis` block of the metadata response
#' @noRd
parse_meta_variables <- function(vars_raw) {
...
#' Parse the `classificacoes` block (with ALL categories)
#' @noRd
parse_meta_classifications <- function(cls_raw) {
  ...
  purrr::map_dfr(cls_raw, function(cls) {
    tibble::tibble(
      ...
      categories = list(parse_meta_categories(cls[["categorias"]]))
    )
  })
}

#' Parse the `categorias` block of one classification
#' @noRd
parse_meta_categories <- function(cats_raw) {
  if (is.null(cats_raw) || length(cats_raw) == 0) {
...
#' Parse the `nivelTerritorial` block of the metadata response
#' @noRd
parse_meta_territorial <- function(nt_raw) {
  if (is.null(nt_raw)) {
    return(list(
...
#' Parse the `periodicidade` block of the metadata response
#' @noRd
parse_meta_periodicity <- function(per_raw) {
  if (is.null(per_raw)) {
    return(
      list(frequency = NA_character_, start = NA_character_,
           end = NA_character_)
    )
  }
...
print.ibge_metadata <- function(x, ...) {
  ...
  cli::cli_text("Periodicity: {x$periodicity$frequency}
                 ({x$periodicity$start} to {x$periodicity$end})")
  levels_str <- toString(x$territorial_level$administrative)
  cli::cli_text("Territorial levels: {levels_str}")
    ...
    for (i in seq_len(nrow(x$classifications))) {
      ...
      if (n_show > 0) {
        for (j in seq_len(n_show)) {
          ...
          cli::cli_text(
            "    {cat_row$category_id}: {cat_row$category_name}{lvl_str}"
          )
        }
        if (n_cats > n_show) {
  ...
  cli::cli_text("")
  cli::cli_text(
    "Use {.code meta$variables} and {.code meta$classifications}
     to access the data."
  )
  cli::cli_text(
    "Use {.code tidyr::unnest(meta$classifications, categories)} to unnest."
  )
