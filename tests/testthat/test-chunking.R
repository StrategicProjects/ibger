...
test_that("resolve_chunk_limit handles TRUE, FALSE and numbers", {
  expect_identical(resolve_chunk_limit(TRUE), 50000)
  expect_null(resolve_chunk_limit(FALSE))
  expect_identical(resolve_chunk_limit(5000), 5000)
...
test_that("split_in_groups splits into consecutive groups", {
  ...
  expect_identical(groups[[1]], 1:3)
  expect_identical(groups[[4]], 10L)
  expect_identical(unlist(groups), 1:10)
...
test_that("units_to_localities_str rebuilds the API syntax", {
  ...
  expect_identical(units_to_localities_str(units), "N3[33,35]|N6[3550308]")
})

# estimate_n_periods -----------------------------------------------------

test_that("estimate_n_periods handles all accepted forms", {
  expect_identical(estimate_n_periods(NULL), 6)
  expect_identical(estimate_n_periods(-4), 4)
  expect_identical(estimate_n_periods("-4"), 4)
  expect_identical(estimate_n_periods(c(2020, 2021)), 2L)
  expect_identical(estimate_n_periods("201701-201712"), 12L)
})
...
test_that("estimate_n_variables uses metadata only when needed", {
  ...
  expect_identical(estimate_n_variables(get_meta, NULL), 8)
  expect_identical(estimate_n_variables(get_meta, "all"), 16)
  expect_identical(estimate_n_variables(get_meta, c(214, 215, 216)), 3L)

  # Explicit variables must not touch metadata
  no_meta <- function() stop("metadata should not be fetched")
  expect_identical(estimate_n_variables(no_meta, c(214, 215)), 2L)
})
...
test_that("estimate_n_categories multiplies category counts", {
  ...
  expect_identical(estimate_n_categories(get_meta, NULL), 1)
  expect_identical(estimate_n_categories(get_meta, list("226" = c(1, 2, 3))), 3)
  expect_identical(estimate_n_categories(get_meta, list("226" = "all")), 34)
  expect_identical(
...
test_that("resolve_localities_for_chunking counts without the API", {
  res <- resolve_localities_for_chunking(9999, "BR")
  expect_identical(res$n, 1)
  expect_null(res$units)

  res <- resolve_localities_for_chunking(
    9999,
    list(N3 = c(33, 35), N6 = 3550308)
  )
  expect_identical(res$n, 3L)
  expect_identical(res$units$level, c("N3", "N3", "N6"))
  expect_identical(res$units$id, c("33", "35", "3550308"))
})
...
test_that("resolve_localities_for_chunking uses the cached level ids", {
  ...
  expect_identical(res$n, 27L)
  expect_identical(res$units$level, rep("N3", 27))
})
...
test_that("build_chunk_plan splits by periods first", {
  ...
  expect_identical(plan[[1]]$periods_str, paste(2000:2005, collapse = "|"))
  expect_identical(plan[[4]]$periods_str, "2018|2019")
  expect_identical(
    plan[[1]]$localities_str,
    paste0("N3[", paste(1:25, collapse = ","), "]")
  )
})
...
test_that("build_chunk_plan splits localities when one period is too large", {
  ...
  expect_identical(plan[[1]]$periods_str, "2000")
  expect_identical(
    plan[[1]]$localities_str,
    paste0("N3[", paste(1:20, collapse = ","), "]")
  )
  expect_identical(plan[[2]]$periods_str, "2000")
  expect_identical(
    plan[[2]]$localities_str,
    paste0("N3[", paste(21:25, collapse = ","), "]")
  )
})
...
test_that("resolve_period_ids resolves last-N and explicit periods", {
  ...
  expect_identical(resolve_period_ids(9999, -3), as.character(2017:2019))
  expect_identical(resolve_period_ids(9999, c(2005, 2007)), c("2005", "2007"))
  expect_identical(
    resolve_period_ids(9999, "2005-2007"),
    as.character(2005:2007)
  )
})
