...
test_that("format_classification builds the API syntax", {
  ...
  expect_identical(
    format_classification(list("226" = c(4844, 96608))),
    "226[4844,96608]"
  )
  expect_identical(format_classification(list("218" = 4780)), "218[4780]")
  expect_identical(
    format_classification(list("226" = c(4844, 96608), "218" = 4780)),
    "226[4844,96608]|218[4780]"
  )
  expect_identical(format_classification(list("226" = "all")), "226[all]")
  expect_identical(format_classification(list("226" = "todos")), "226[all]")
})
...
test_that("format_localities accepts all documented forms", {
  expect_identical(format_localities("BR"), "BR")
  expect_identical(format_localities("N3"), "N3")
  expect_identical(format_localities(c("N3", "N6")), "N3|N6")
  expect_identical(
    format_localities(list(N6 = c(3550308, 3304557))),
    "N6[3550308,3304557]"
  )
  expect_identical(
...
test_that("format_periods handles last-N, vectors and ranges", {
  expect_identical(format_periods(NULL), "-6")
  expect_identical(format_periods(-6), "-6")
  expect_identical(format_periods(c(201701, 201702)), "201701|201702")
  expect_identical(format_periods("201701-201712"), "201701-201712")
})

# format_variable -----------------------------------------------------------

test_that("format_variable handles NULL, all and vectors", {
  expect_identical(format_variable(NULL), "allxp")
  expect_identical(format_variable("all"), "all")
  expect_identical(format_variable("todas"), "all")
  expect_identical(format_variable(c(284, 285)), "284|285")
  expect_identical(format_variable(214), "214")
})
...
test_that("pluck_chr walks nested lists safely", {
  ...
  expect_identical(pluck_chr(x, "a", "b"), "value")
  expect_identical(pluck_chr(x, "n"), "42")
  expect_true(is.na(pluck_chr(x, "missing")))
  expect_identical(pluck_chr(x, "missing", .default = "d"), "d")
})
